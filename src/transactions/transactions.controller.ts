...
export class TransactionsController {
  ...
  @ApiOperation({
    summary:
      'Create a new transaction for an authenticated user against an account',
  })
  @ApiBadRequestResponse({
  ...
  @ApiOperation({
    summary:
      'Fetch all transactions for a given user against a specific account',
  })
  @ApiResponse({
    status: 200,
    description: 'List of accounts',
  ...
  async getTransaction(
    ...
    return this.getTransactionHandler.handle({
      userId: user.uuid,
      accountId,
      transactionId,
    });
