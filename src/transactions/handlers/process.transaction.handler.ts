...
export class ProcessTransactionHandler extends TransactionHandler {
  ...
  async handle(request: TransactionRequest) {
    ...
    try {
      ...
      const completedTransactions = await queryRunner.manager.find(
        TransactionEntity,
        {
          where: {
      ...
      const currentAccountBalance = completedTransactions.reduce(
        balanceReducer,
        0,
      );
      const newAccountBalance = balanceReducer(
